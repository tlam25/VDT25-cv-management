...
@router.get("/employees_without_cv")
async def get_employees_without_cv(current_user: UserResponse = Depends(get_current_user), db: Session = Depends(get_db)):
    if RoleType.pm.value not in current_user.roles:
        raise HTTPException(
            status_code=403,
            detail="User does not have permission to access this resource"
    ...
    # Lấy danh sách emp_id có trong CvItem
    cv_employee_ids = db.query(CvItem.emp_id).subquery().select()  # Explicit subquery handling

    # Lấy danh sách proj_id mà PM quản lý
    subquery_project = (
        db.query(Project.proj_id)
        .filter(Project.pm_id == pm.emp_id)
        .subquery()
        .select()  # Explicit subquery handling
    )

    # Lấy danh sách emp_id làm việc trong các dự án của PM
    subquery_emps_works_on_project = (
        db.query(WorksOnProject.emp_id)
        .filter(WorksOnProject.proj_id.in_(subquery_project))
        .subquery()
    ...
    # Truy vấn nhân viên không có CV, làm việc trong dự án của PM, trạng thái active
    results = (
        db.query(Employee.emp_id, Employee.first_name, Employee.last_name, Employee.dob, Employee.gender,
                 subquery_dept.label("department"), Employee.start_date, Employee.phone, Employee.email, Employee.address)
        .filter(
            Employee.emp_id.in_(subquery_emps_works_on_project),
            Employee.status == EmployeeStatus.active,
            Employee.emp_id.notin_(cv_employee_ids)
        )
        .all()
    )
    ...
    return {"employees_without_cv": emp_list}