...
async def get_employees_without_cv(current_user: UserResponse = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    # Get emp_ids with CVs
    cv_employee_ids = db.query(CvItem.emp_id).subquery().select()  
    ...
    for emp in results:
        subquery_role = (
            ...
            .filter(Role.role_id.in_(
                db.query(HasRole.role_id)
                .filter(HasRole.emp_id == emp.emp_id)  # Scope to specific employee
                .subquery()
                .select()  # Explicit subquery handling
            ))
            .subquery()
        )
        emp_roles = db.query(subquery_role).all()
